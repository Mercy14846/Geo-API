...
def _fetch_hdx_boundaries(iso3: str, level: int, crs: str) -> gpd.GeoDataFrame:
    ...
    try:
        ...
        raise ImportError(
            "hdx-python-api is required for HDX source. Install it with: pip install hdx-python-api"
        )
    ...
    dataset = datasets[0]
    resources = [
        r for r in dataset.get_resources() if r.get_file_type() in ("SHP", "GEOJSON", "GPKG", "ZIP")
    ]
    if not resources:
        raise DataNotFoundError(
